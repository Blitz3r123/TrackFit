...
import { Text, Dimensions, StyleSheet, View, TextInput, Alert, ScrollView } from 'react-native';
			...
			if(!dateExists){
				...
				*/
				if(dateObj.items.length == 0){
					...
					dateItem.key = dateObj.items.sort((a, b) => {return a.key - b.key})[dateObj.items.length - 1].key + 1;
				}
									...
									return (
										<Card key={date.key}>
											...
											<CardItem style={{paddingTop: 0, paddingLeft: 0, marginTop: -20}}>
												...
												{
													date.items.sort((a, b) => {b.key - a.key}).reverse().map(item => {
														return (
